...
const SAMPLE_LISTINGS = [
  ...
  },
  {
    ...
    bio: "Sharing my beautiful Clifton flat. Walking distance to amazing schools and parks. My daughter is independent and loves art. I'm a nurse working shifts so I'd love someone who's flexible and understanding.",
  ...
  },
  {
    ...
    bio: "East London terrace with two lively 6-year-olds. We're loud, loving and fun. Looking for someone with kids of a similar age — playdates built in! Close to the park and brilliant primary schools.",
...
const SAMPLE_BUDDIES = [
  {
    ...
    message: "Hi, I'm Tania, 25 years old with 2 young kids age 5 and 7 and I'm looking for another single mum with similar age to share an accommodation preferably SOR around Melville area. I work part-time as a teacher and love weekend beach walks with the kids 🌊",
  ...
  },
  {
    ...
    message: "Single mum of 2, looking for another mum to find a 3–4 bed house together around Freo or Cockburn. I'm fun, tidy and love a glass of wine after bedtime 🍷 Budget up to $1,100/month all up.",
  ...
  },
  {
    ...
    message: "Looking for a calm, kind mum to find a home together NOR or inner suburbs. My daughter is 6 and in kindy. I'm a nurse doing rotating shifts so I need someone flexible and understanding. Happy to consider anywhere from Subi to Scarborough.",
  ...
  },
  {
    ...
    message: "Mum of 3 looking for another mum — ideally with kids of similar ages so they can entertain each other! 😄 I'm looking around Vic Park, Carlisle or Burswood. I love cooking, hate mess, and my kids are pretty well-behaved (most of the time 😅).",
...
export default function App() {
  ...
  const sendMsg = () => {
    ...
    setNewMsg("");
    setTimeout(() => setMessages(p => ({ ...p, [id]: [...(p[id]||[]), { from:"them", text:"Thanks for reaching out! Let's chat more 💛", time:"Now" }] })), 1200);
  };

  const submitBuddy = () => {
    if (!buddyForm.name || !buddyForm.location || !buddyForm.message) return toast("Please fill name, location & message");
      ...
      <div style={{ padding:"16px 18px" }}>
        <div style={{ display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:8 }}>
          <div style={{ display:"flex",gap:10,alignItems:"center" }}>
            ...
            <div>
              <div style={{ fontFamily:"'DM Sans',sans-serif",fontWeight:600,color:"#3d2c1e",fontSize:14 }}>{l.name} {l.verified&&<span style={{color:"#c4704a",fontSize:11}}>✓</span>}</div>
    ...
    <div style={{ fontFamily:"'Georgia',serif",background:"#faf7f2",minHeight:"100vh",maxWidth:430,margin:"0 auto",position:"relative",overflowX:"hidden" }}>
      ...
      {view==="home" && (
        <div className="fade-in">
          <div style={{ background:"linear-gradient(160deg,#3d2c1e 0%,#7a4430 50%,#c4704a 100%)",padding:"50px 26px 46px",position:"relative",overflow:"hidden" }}>
            ...
            <h1 className="pf" style={{ fontSize:31,color:"#fff",lineHeight:1.2,marginBottom:12,fontWeight:400 }}>Find your <em>people</em>,<br/>share your home</h1>
          ...
          <div style={{ padding:"22px 18px 0" }}>
            ...
            <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:22 }}>
              <div onClick={()=>setView("browse")} style={{ background:"linear-gradient(135deg,#fff5ef,#ffe8d8)",borderRadius:18,padding:"18px 14px",cursor:"pointer",boxShadow:"0 2px 12px rgba(196,112,74,.1)" }}>
                ...
                <div style={{ fontSize:11,color:"#9a8878",fontFamily:"'DM Sans',sans-serif",lineHeight:1.4 }}>List or find a room in someone's home</div>
              </div>
          ...
          <div style={{ padding:"6px 18px 100px" }}>
            ...
            {[["🔍","Browse & search","Filter by location, budget and kids' ages."],["🤝","Find a buddy","Team up — twice the bargaining power."],["💬","Connect & chat","Message before committing to anything."],["🏡","Move in together","Share costs, support, and build a home."]].map(([icon,title,desc])=>(
          ...
          <div style={{ padding:"50px 18px 12px",background:"#f0f4ff",borderBottom:"1px solid #d8e2f8",position:"sticky",top:0,zIndex:100 }}>
            <div style={{ display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:4 }}>
              <div>
                ...
                <p style={{ fontFamily:"'DM Sans',sans-serif",fontSize:12,color:"#6a7aaa",marginTop:2 }}>Team up to find & rent a home together</p>
              </div>
      ...
      {view==="detail" && selectedListing && (
        <div className="slide-up">
          ...
          <div style={{ padding:"20px 18px 100px" }}>
            ...
            {selectedListing.verified && <div style={{ background:"#f0f9f0",borderRadius:10,padding:"9px 13px",marginBottom:12,display:"flex",gap:7,alignItems:"center" }}><span>✅</span><span style={{ fontFamily:"'DM Sans',sans-serif",fontSize:12,color:"#3a7a3a" }}>Identity verified by SoloNest</span></div>}
        ...
        <div className="fade-in">
          ...
          <div style={{ padding:"14px 18px 100px" }}>
            ...
            {buddies.filter(b=>savedBuddies.includes(b.id)).map(b=>(
              <div key={b.id} className="card" style={{ marginBottom:10,cursor:"pointer",padding:14,background:"#f8faff",border:"1.5px solid #d8e2f8",borderRadius:16,boxShadow:"none" }} onClick={()=>{setSelectedBuddy(b);setView("buddyDetail")}}>
                <div style={{ display:"flex",gap:11,alignItems:"center" }}>
      ...
      {view==="profile" && (
        <div className="fade-in">
          ...
          <div style={{ padding:"20px 18px 100px" }}>
            {[["🏠","My Room Listings","Manage your listings",()=>setView("browse")],["🤝","My Buddy Ads","Find mums to rent with",()=>setView("buddy")],["🤍","Saved","Rooms & buddies saved",()=>setView("saved")],["💬","Messages","Chat with other mums",()=>setView("messages")],["✏️","Edit Profile","Update your details",()=>toast("Coming soon!")],["🔒","Privacy & Safety","Your safety tools",()=>toast("Coming soon!")],["❓","Help & Support","Get help anytime",()=>toast("Coming soon!")]].map(([icon,title,sub,action])=>(
        ...
        <div style={{ position:"fixed",inset:0,background:"rgba(0,0,0,.5)",zIndex:500,display:"flex",alignItems:"flex-end" }} onClick={()=>setShowBuddyForm(false)}>
          <div className="slide-up" style={{ background:"#f0f4ff",borderRadius:"24px 24px 0 0",padding:"22px 18px 40px",width:"100%",maxHeight:"93vh",overflowY:"auto" }} onClick={e=>e.stopPropagation()}>
            <div style={{ display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:5 }}>
              ...
              <button onClick={()=>setShowBuddyForm(false)} style={{ background:"#d8e2f8",border:"none",borderRadius:"50%",width:28,height:28,cursor:"pointer",fontSize:14 }}>✕</button>
            </div>
            <p style={{ fontFamily:"'DM Sans',sans-serif",fontSize:12,color:"#6a7aaa",marginBottom:16 }}>Tell other mums who you are and what you're looking for so they can reach out and team up with you.</p>
            <div style={{ display:"flex",flexDirection:"column",gap:11 }}>
              {[["Your name *","name","text","e.g. Tania W."],["Your age","age","number","e.g. 25"],["Preferred location *","location","text","e.g. Melville, WA"],["Max budget ($/mo)","budget","number","e.g. 900"],["Kids' ages (comma separated)","childrenAges","text","e.g. 5, 7"]].map(([label,field,type,ph])=>(
                <div key={field}>
              ...
              <div>
                <div style={{ fontFamily:"'DM Sans',sans-serif",fontSize:12,color:"#3a4a6a",marginBottom:4 }}>Your message * <span style={{color:"#9a9ab0"}}>(introduce yourself & what you're after)</span></div>
                <textarea rows={5} placeholder="e.g. Hi, I'm Tania, 25 years old with 2 young kids age 5 and 7 and I'm looking for another single mum with similar age to share accommodation preferably SOR around Melville area." value={buddyForm.message} onChange={e=>setBuddyForm(p=>({...p,message:e.target.value}))} style={{ resize:"none",borderColor:"#d8e2f8",background:"#fff" }}/>
              </div>
              <div>
        ...
        <div style={{ position:"fixed",inset:0,background:"rgba(0,0,0,.5)",zIndex:500,display:"flex",alignItems:"flex-end" }} onClick={()=>setShowListingForm(false)}>
          <div className="slide-up" style={{ background:"#faf7f2",borderRadius:"24px 24px 0 0",padding:"22px 18px 40px",width:"100%",maxHeight:"93vh",overflowY:"auto" }} onClick={e=>e.stopPropagation()}>
            <div style={{ display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16 }}>
              ...
              <button onClick={()=>setShowListingForm(false)} style={{ background:"#e8ddd5",border:"none",borderRadius:"50%",width:28,height:28,cursor:"pointer",fontSize:14 }}>✕</button>
            </div>
            <div style={{ display:"flex",flexDirection:"column",gap:11 }}>
              {[["Your name *","name","text","e.g. Sarah M."],["Location *","location","text","e.g. Brighton, UK"],["Monthly rent (£) *","rent","number","e.g. 650"],["Number of bedrooms","bedrooms","number","e.g. 3"],["Your children","children","text","e.g. 2 kids, ages 4 & 7"]].map(([label,field,type,ph])=>(
                <div key={field}>
              ...
              <div>
                <div style={{ fontFamily:"'DM Sans',sans-serif",fontSize:12,color:"#6b5a50",marginBottom:4 }}>About your home & lifestyle</div>
                <textarea rows={4} placeholder="Tell us about yourself, your home and what you're looking for…" value={listingForm.bio} onChange={e=>setListingForm(p=>({...p,bio:e.target.value}))} style={{ resize:"none" }}/>
              </div>
              <div>
